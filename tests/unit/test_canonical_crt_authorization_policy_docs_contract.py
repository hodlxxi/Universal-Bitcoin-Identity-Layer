...
def test_primary_policy_document_is_published_and_dormant():
    ...
    assert "| FULL/LIMITED authorization policy mapping " in status and "| IMPLEMENTED_DORMANT |" in status
    public_row = next(line for line in status.splitlines() if "Public why-FULL proof" in line)
