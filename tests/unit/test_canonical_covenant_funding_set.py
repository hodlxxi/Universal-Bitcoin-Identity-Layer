...
from app.services.canonical_covenant_funding_set import (
    CanonicalCovenantFundingSet, CovenantFundingSetLifecycle,
    InvalidCanonicalCovenantFundingSet, RecognizedCovenantFundingOutpoint,
    canonical_covenant_funding_set_bytes, canonical_covenant_funding_set_sha256,
    create_canonical_covenant_funding_set, parse_canonical_covenant_funding_set,
...
from app.services.trusted_covenant_registration import (
    RegisteredCovenantOutpoint, TrustedCovenantRegistrationLifecycle,
...
def registration(state=TrustedCovenantRegistrationLifecycle.ACTIVE):
    pair = validate_mirrored_covenant_pair(LEG1, LEG2, subject_pubkey=SUBJECT,
        allowed_delta_profiles=(CovenantDeltaProfile.LEGACY_777,))
    return create_trusted_covenant_registration(pair, (
        RegisteredCovenantOutpoint(CovenantDirection.INCOMING, "a"*64, 0, 133129, pair.incoming_leg_script_sha256, "c"*64),
        RegisteredCovenantOutpoint(CovenantDirection.OUTGOING, "d"*64, 1, 122223, pair.outgoing_leg_script_sha256),
    ), registration_id="00000000-0000-4000-8000-000000000001", lifecycle_state=state,
       registered_at=NOW, lifecycle_changed_at=NOW)
...
def funding(reg=None, reverse=False):
    ...
    values = (
        RecognizedCovenantFundingOutpoint(CovenantDirection.INCOMING, "a"*64, 0, 133129, pair.incoming_leg_script_sha256, "c"*64),
        RecognizedCovenantFundingOutpoint(CovenantDirection.INCOMING, "b"*64, 2, 44648, pair.incoming_leg_script_sha256),
        RecognizedCovenantFundingOutpoint(CovenantDirection.INCOMING, "c"*64, 3, 122223, pair.incoming_leg_script_sha256),
        RecognizedCovenantFundingOutpoint(CovenantDirection.OUTGOING, "d"*64, 1, 122223, pair.outgoing_leg_script_sha256),
        RecognizedCovenantFundingOutpoint(CovenantDirection.OUTGOING, "e"*64, 4, 177777, pair.outgoing_leg_script_sha256),
    )
    return create_canonical_covenant_funding_set(
        funding_set_id="00000000-0000-4000-8000-000000000010", trusted_registration=reg,
        lifecycle_state=CovenantFundingSetLifecycle.EFFECTIVE, created_at=NOW,
        lifecycle_changed_at=NOW, effective_at=NOW,
...
def test_multi_utxo_vector_canonical_roundtrip_digest_and_immutability():
    ...
    assert [x.amount_sats for x in value.recognized_outpoints if x.direction is CovenantDirection.INCOMING] == [133129, 44648, 122223]
    assert [x.amount_sats for x in value.recognized_outpoints if x.direction is CovenantDirection.OUTGOING] == [122223, 177777]
...
@pytest.mark.parametrize(("field", "value"), (
    ("txid", "g"*64), ("txid", "A"*64), ("vout", True), ("vout", 1.0),
    ("vout", -1), ("vout", 4294967296), ("witness_script_sha256", "g"*64),
    ("descriptor_sha256", "G"*64),
))
def test_outpoint_fields_reject_malformed_values_and_coercion(field, value):
    values = {"direction": CovenantDirection.INCOMING, "txid": "a"*64, "vout": 0,
              "amount_sats": 1, "witness_script_sha256": "b"*64,
...
@pytest.mark.parametrize(("state", "effective", "successor"), (
...
def test_lifecycle_consistency_fails_closed(state, effective, successor):
    ...
    with pytest.raises(InvalidCanonicalCovenantFundingSet):
        replace(value, lifecycle_state=state, effective_at=effective,
                superseded_by_funding_set_id=successor)
...
@pytest.mark.parametrize("field", (
    "trusted_registration_id", "trusted_registration_sha256", "pair_sha256",
    "subject_xonly_pubkey", "counterparty_xonly_pubkey",
))
...
def test_duplicate_wrong_script_missing_direction_and_anchor_fail_closed():
    ...
    with pytest.raises(InvalidCanonicalCovenantFundingSet):
        validate_funding_set_registration(replace(value, recognized_outpoints=(value.recognized_outpoints[0], wrong) + value.recognized_outpoints[2:]), registration())
    with pytest.raises(InvalidCanonicalCovenantFundingSet):
        validate_funding_set_registration(replace(value, recognized_outpoints=value.recognized_outpoints[1:]), registration())
    with pytest.raises(InvalidCanonicalCovenantFundingSet):
        CanonicalCovenantFundingSet(*(getattr(value, f) for f in list(CanonicalCovenantFundingSet.__dataclass_fields__)[:-1]),
...
def test_one_incoming_two_outgoing_is_structurally_valid_when_anchor_exact():
    ...
    value = create_canonical_covenant_funding_set(
        funding_set_id="00000000-0000-4000-8000-000000000011", trusted_registration=reg,
        lifecycle_state=CovenantFundingSetLifecycle.EFFECTIVE, created_at=NOW,
        lifecycle_changed_at=NOW, effective_at=NOW, recognized_outpoints=(
            RecognizedCovenantFundingOutpoint(CovenantDirection.INCOMING, "a"*64, 0, 133129, p.incoming_leg_script_sha256, "c"*64),
            RecognizedCovenantFundingOutpoint(CovenantDirection.OUTGOING, "d"*64, 1, 122223, p.outgoing_leg_script_sha256),
            RecognizedCovenantFundingOutpoint(CovenantDirection.OUTGOING, "e"*64, 2, 50000, p.outgoing_leg_script_sha256),))
