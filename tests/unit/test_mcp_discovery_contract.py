...
def test_mcp_gate_explicit_true_enables_metadata(client, monkeypatch):
...
def test_mcp_gate_false_and_malformed_values_remain_disabled(client, monkeypatch):
    for value in ["false", "0", "no", "", "definitely"]:
        monkeypatch.setenv("HODLXXI_MCP_PUBLIC_ENABLED", value)
        assert client.get("/.well-known/mcp.json").get_json()["enabled"] is False
...
def test_mcp_server_card_truthful_sidecar_contract(client):
    ...
    assert data["serverInfo"] == {"name": "HODLXXI Read-Only", "version": "0.1.0"}
