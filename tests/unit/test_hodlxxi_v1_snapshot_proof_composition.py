...
def test_genesis_rejects_observed_and_active_rejects_not_required():
    ...
    ordinary = plan()
    observed = TrustedCrtBitcoinObservationSnapshotAdapter(rpc=RPC(ordinary), clock=Clock()).observe(
        source_plan=ordinary
    )
    with pytest.raises(InvalidHodlxxiV1SnapshotProofComposition):
        HodlxxiV1SnapshotProofCompositionRequest(
            genesis, observed, NOW + timedelta(seconds=2), NOW + timedelta(minutes=5)
        )
...
def test_request_and_nested_result_subclasses_are_rejected():
    class Plan(type(genesis_plan())):
        pass

    class Resolution(TrustedCrtBitcoinObservationResolution):
        pass

    class Result(HodlxxiV1SnapshotProofCompositionResult):
        pass
...
def forged_result(result, **changes):
    ...
    values["result_sha256"] = composition_module._result_sha256_unchecked(SimpleNamespace(**values))
...
def test_recomputed_outer_digest_rejects_valid_nested_object_substitution(kind):
    ...
    changes = {
        ...
        "proof": {
            "authorization_proof": other.authorization_proof,
...
def test_result_detaches_caller_aliases_and_serializer_revalidates(nested):
    ...
    object.__setattr__(
        source_plan if nested == "source_plan" else resolution,
...
def test_composer_rejects_nonexact_observed_relation_matrix(shape):
    ...
    other_plan = plan()
    foreign = (
        TrustedCrtBitcoinObservationSnapshotAdapter(rpc=RPC(other_plan), clock=Clock())
        .observe(source_plan=other_plan)
        .snapshot.observed_relations[0]
    )
    changed = {
        "missing": relations[:-1],
...
def test_lifecycle_controlled_registration_accepts_none_but_never_active(lifecycle):
    ...
    record = replace(item.record, trusted_registration_sha256=trusted_registration_sha256(registration))
    result = evaluate_canonical_admission_edge_current(
        record,
