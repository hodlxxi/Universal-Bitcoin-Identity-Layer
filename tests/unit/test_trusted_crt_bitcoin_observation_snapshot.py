...
class RPC:
    def __init__(self, source_plan, *, heights=(900000, 900000), hashes=(BLOCK, BLOCK),
                 blockhashes=(BLOCK, BLOCK), null=False, error=None, descriptor_text=None):
        self.calls = []
        self.heights = list(heights); self.hashes = list(hashes); self.blockhashes = list(blockhashes)
        self.null = null; self.error = error; self.descriptor_text = descriptor_text
        self.bindings = {}
    ...
    def getbestblockhash(self):
        self.calls.append(("getbestblockhash",)); return self.hashes.pop(0)

    def getblockhash(self, height):
        self.calls.append(("getblockhash", height)); return self.blockhashes.pop(0)
    ...
    def gettxout(self, txid, vout, include_mempool):
        ...
        response = {"bestblock": BLOCK, "confirmations": 3,
...
class Clock:
    def __init__(self, values=(NOW, NOW + timedelta(seconds=1)), error=None):
        self.values = list(values); self.calls = 0; self.error = error
    def __call__(self):
        self.calls += 1
...
def test_snapshot_rejects_source_plan_subclass_and_mutation_with_recomputed_digest():
    ...
    class Sub(TrustedCrtAuthorizationSourcePlan): pass
...
def test_required_subset_rejects_nonrequired_inclusion_and_required_omission():
    ...
    with pytest.raises(TrustedCrtBitcoinObservationUnavailable):
        forged_snapshot(snapshot, observed_relations=(snapshot.observed_relations[0], nonrequired_relation,
                                                       snapshot.observed_relations[1]))
    with pytest.raises(TrustedCrtBitcoinObservationUnavailable):
...
def test_observed_resolution_revalidates_snapshot_plan_digest():
    ...
    with pytest.raises(TrustedCrtBitcoinObservationUnavailable):
        TrustedCrtBitcoinObservationResolution(
            TrustedCrtBitcoinObservationState.OBSERVED, "0" * 64, snapshot,
        )
...
def test_composed_observer_rejects_nonnull_response_mismatches(failure):
    ...
    def invalid(*args):
        ...
        if failure == "bestblock": response["bestblock"] = "e" * 64
        if failure == "amount": response["value"] += Decimal("0.00000001")
        if failure == "script": response["scriptPubKey"]["hex"] = "00"
        return response
...
def test_composed_observer_enforces_descriptor_digest():
    ...
    registration = replace(
        source_registration,
        outpoints=(replace(source_registration.outpoints[0], descriptor_sha256=descriptor_digest),
...
def test_clock_exception_is_sanitized_and_rpc_base_exceptions_propagate():
    ...
    with pytest.raises(TrustedCrtBitcoinObservationUnavailable) as caught:
        TrustedCrtBitcoinObservationSnapshotAdapter(
            rpc=RPC(p), clock=Clock(error=RuntimeError(secret)),
...
def test_snapshot_digest_changes_for_global_hash_and_plan_bound_sources():
    ...
    assert other_plan.observed_relations[-1].trusted_outpoints_sha256 != first.observed_relations[0].trusted_outpoints_sha256
    assert other_plan.observed_relations[-1].relation_evaluation_sha256 != first.observed_relations[0].relation_evaluation_sha256


def test_public_contracts_and_serializers_reject_subclasses():
    snapshot = observe(plan())[0].snapshot
    relation = snapshot.observed_relations[0]
    class RelationSubclass(TrustedCrtObservedLineageRelation): pass
    ...
    class SnapshotSubclass(TrustedCrtBitcoinObservationSnapshot): pass
...
def test_null_is_authoritative_negative_and_canonical_forms_are_deterministic():
    ...
    assert trusted_crt_outpoint_manifest_bytes(relation.trusted_outpoints) == trusted_crt_outpoint_manifest_bytes(relation.trusted_outpoints)
    assert trusted_crt_bitcoin_observation_snapshot_bytes(result.snapshot) == trusted_crt_bitcoin_observation_snapshot_bytes(result.snapshot)
...
def test_completed_before_started_and_public_digest_forgery_fail_closed():
    ...
    with pytest.raises(TrustedCrtBitcoinObservationUnavailable):
        TrustedCrtBitcoinObservationSnapshotAdapter(
            rpc=rpc, clock=Clock((NOW, NOW - timedelta(seconds=1)))) .observe(source_plan=p)
    snapshot = observe(p)[0].snapshot
    with pytest.raises(TrustedCrtBitcoinObservationUnavailable):
...
def test_rpc_dictionary_alias_is_not_retained():
    ...
    original = rpc.gettxout
    def capture(*args):
        value = original(*args); responses.append(value); return value
