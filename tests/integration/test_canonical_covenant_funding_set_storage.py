...
from app.services.canonical_covenant_funding_set import (
    CovenantFundingSetLifecycle, RecognizedCovenantFundingOutpoint,
    canonical_covenant_funding_set_bytes, create_canonical_covenant_funding_set,
)
...
from app.services.trusted_covenant_registration import RegisteredCovenantOutpoint, TrustedCovenantRegistrationLifecycle, create_trusted_covenant_registration
...
def registration(state=TrustedCovenantRegistrationLifecycle.ACTIVE):
    p = validate_mirrored_covenant_pair(LEG1, LEG2, subject_pubkey=SUBJECT, allowed_delta_profiles=(CovenantDeltaProfile.LEGACY_777,))
    return create_trusted_covenant_registration(p, (
       ...
       lifecycle_state=state, registered_at=NOW, lifecycle_changed_at=NOW)


class Registrations:
    def __init__(self, value): self.value = value
    def get(self, identifier): return self.value if identifier == self.value.registration_id else None


def funding(identifier="00000000-0000-4000-8000-000000000010", state=CovenantFundingSetLifecycle.EFFECTIVE):
    reg = registration(); p = reg.mirrored_pair
    return create_canonical_covenant_funding_set(
        funding_set_id=identifier, trusted_registration=reg, lifecycle_state=state,
        created_at=NOW, lifecycle_changed_at=NOW,
        effective_at=NOW if state in (CovenantFundingSetLifecycle.EFFECTIVE, CovenantFundingSetLifecycle.SUPERSEDED) else None,
        superseded_by_funding_set_id="00000000-0000-4000-8000-000000000099" if state is CovenantFundingSetLifecycle.SUPERSEDED else None,
        recognized_outpoints=(
            RecognizedCovenantFundingOutpoint(CovenantDirection.INCOMING, "a"*64, 0, 100000, p.incoming_leg_script_sha256),
            RecognizedCovenantFundingOutpoint(CovenantDirection.OUTGOING, "b"*64, 1, 100000, p.outgoing_leg_script_sha256),
            RecognizedCovenantFundingOutpoint(CovenantDirection.OUTGOING, "c"*64, 2, 50000, p.outgoing_leg_script_sha256),))
...
def store(tmp_path):
    ...
    return engine, factory, SqlAlchemyCanonicalCovenantFundingSetRepository(factory, trusted_registration_repository=Registrations(registration()))


def test_append_get_list_and_resolve(store):
    _, _, repo = store; expected = funding()
...
def test_list_revalidates_effective_authority_and_sanitizes_failure(store):
    ...
    repo._trusted_registration_repository.value = registration(
...
def test_unique_effective_and_historical_coexistence(store):
    ...
    for number, state in enumerate((CovenantFundingSetLifecycle.PROPOSED, CovenantFundingSetLifecycle.DISPUTED,
            CovenantFundingSetLifecycle.REVOKED, CovenantFundingSetLifecycle.SUPERSEDED), 20):
...
def test_zero_effective_and_tampered_scalar_or_child_fail_closed(store):
    ...
    value = funding(); repo.append(value)
    with factory() as session:
        session.execute(text("UPDATE canonical_covenant_funding_sets SET pair_sha256 = :x"), {"x": "f"*64}); session.commit()
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
...
def test_boundary_vout_persists_and_database_rejects_non_hex(store):
    ...
    boundary = RecognizedCovenantFundingOutpoint(
        CovenantDirection.INCOMING, "f"*64, 4294967295, 1,
        p.incoming_leg_script_sha256)
    value = type(value)(*(getattr(value, f) for f in list(type(value).__dataclass_fields__)[:-1]),
    ...
    with factory() as session:
        ...
        with pytest.raises(IntegrityError):
            session.execute(text("UPDATE canonical_covenant_funding_outpoints SET txid = :x WHERE txid = :old"),
                            {"x": "g"*64, "old": "f"*64})
            session.commit()
        session.rollback()
...
def test_missing_extra_and_noncanonical_child_rows_fail_closed(store):
    _, factory, repo = store; value = funding(); repo.append(value)
    with factory() as session:
        session.execute(text("DELETE FROM canonical_covenant_funding_outpoints WHERE txid = :x"), {"x": "c"*64}); session.commit()
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
    with factory() as session:
        session.execute(text("INSERT INTO canonical_covenant_funding_outpoints (funding_set_id,direction,txid,vout,amount_sats,witness_script_sha256) VALUES (:set_id,'outgoing',:txid,9,1,:script)"),
                        {"set_id": value.funding_set_id, "txid": "f"*64,
                         "script": registration().mirrored_pair.outgoing_leg_script_sha256}); session.commit()
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
    with factory() as session:
        session.execute(text("DELETE FROM canonical_covenant_funding_outpoints WHERE txid = :x"), {"x": "f"*64})
        session.execute(text("UPDATE canonical_covenant_funding_outpoints SET direction = 'incoming' WHERE txid = :x"), {"x": "b"*64}); session.commit()
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
...
def test_canonical_json_tamper_and_authoritative_source_change_fail_closed(store):
    _, factory, repo = store; value = funding(); repo.append(value)
    with factory() as session:
        session.execute(text("UPDATE canonical_covenant_funding_sets SET canonical_record_json = canonical_record_json || ' '")); session.commit()
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
    with factory() as session:
        session.execute(text("UPDATE canonical_covenant_funding_sets SET canonical_record_json = :canonical"),
                        {"canonical": canonical_covenant_funding_set_bytes(value).decode("ascii")}); session.commit()
    repo._trusted_registration_repository.value = registration(TrustedCovenantRegistrationLifecycle.REVOKED)
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.resolve_effective(value.trusted_registration_id)
    with factory() as session:
        session.execute(text("UPDATE canonical_covenant_funding_sets SET pair_sha256 = :x"), {"x": value.pair_sha256})
        session.execute(text("UPDATE canonical_covenant_funding_outpoints SET amount_sats = amount_sats + 1 WHERE txid = :x"), {"x": "c"*64}); session.commit()
    with pytest.raises(CanonicalCovenantFundingSetStorageError): repo.get(value.funding_set_id)
...
def test_model_migration_parity_and_partial_index():
    ...
    index = {x.name: x for x in CanonicalCovenantFundingSetRow.__table__.indexes}["uq_funding_set_effective_registration"]
    assert index.unique and index.dialect_options["sqlite"]["where"] is not None and index.dialect_options["postgresql"]["where"] is not None
    ...
    assert isinstance(CanonicalCovenantFundingOutpointRow.__table__.c.id.type.dialect_impl(
        create_engine("sqlite://").dialect), Integer)
    ...
    assert set(CanonicalCovenantFundingSetRow.__table__.columns.keys()) == {
        "funding_set_id", "schema", "funding_set_version", "trusted_registration_id",
        "trusted_registration_sha256", "pair_sha256", "subject_xonly_pubkey",
        "counterparty_xonly_pubkey", "lifecycle_state", "created_at",
        "lifecycle_changed_at", "effective_at", "superseded_by_funding_set_id",
        "canonical_funding_set_sha256", "canonical_record_json",
    }
    assert set(CanonicalCovenantFundingOutpointRow.__table__.columns.keys()) == {
        "id", "funding_set_id", "direction", "txid", "vout", "amount_sats",
        "witness_script_sha256", "descriptor_sha256",
    }
    ...
    assert {"ck_funding_set_source_digests", "ck_funding_set_participants", "ck_funding_set_digest"} <= parent_constraints
    assert {"ck_funding_outpoint_txid", "ck_funding_outpoint_script", "ck_funding_outpoint_descriptor"} <= child_constraints
    assert "id BIGSERIAL PRIMARY KEY" in sql and "vout BIGINT" in sql
    assert "txid ~ '^[0-9a-f]{64}$'" in sql
    for name in parent_constraints | child_constraints | {
        "uq_funding_outpoint_identity", "uq_funding_set_effective_registration",
        "idx_funding_set_registration", "idx_funding_outpoint_set",
    }:
        if name is not None:
            assert name in sql
...
def test_database_rejects_disputed_effective_time_outside_lifecycle_bounds(store, effective):
    ...
    with factory() as session:
        with pytest.raises(IntegrityError):
            session.execute(text("UPDATE canonical_covenant_funding_sets SET lifecycle_state = 'disputed', effective_at = :effective"),
                            {"effective": effective})
            session.commit()
        session.rollback()
...
def test_effective_ambiguity_and_write_failure_are_sanitized(monkeypatch):
    ...
    class FailedSession:
        ...
        def commit(self): raise RuntimeError("database detail must be hidden")
