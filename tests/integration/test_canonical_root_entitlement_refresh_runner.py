...
def test_parser_and_guard_need_no_application_or_network(tmp_path):
    request = runner.parse_runner_argv([
        ...
        "--subject", "a" * 64,
        "--commit", "--lock-directory", str(tmp_path),
    ])
...
def test_one_shot_runner_composes_real_adapters_append_then_unchanged(tmp_path, monkeypatch, limited):
    ...
    funding = SqlAlchemyCanonicalCovenantFundingSetRepository(
        factory, trusted_registration_repository=trusted
    )
    ...
    relation = EdgeLocalCovenantRelationResult(
        ...
        5, 1 if limited else 5, now, 900000, 300000, 0 if limited else 300000, not limited,
    ...
    dependencies = lambda _mode: {
        "genesis_repository": genesis, "admission_edge_repository": admission,
        "root_registration_binding_repository": binding,
        "trusted_registration_repository": trusted, "funding_set_repository": funding,
        "evidence_repository": evidence, "rpc_factory": lambda: (_ for _ in ()).throw(AssertionError("offline")),
    }
    request = runner.parse_runner_argv([
        "--graph", graph, "--subject", subject, "--commit",
        "--lock-directory", str(tmp_path),
    ])
