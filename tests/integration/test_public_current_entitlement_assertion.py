...
def test_malformed_evidence_source_fails_closed_without_leakage(client, monkeypatch, evidence_source, private_marker):
...
def test_malformed_observed_at_fails_closed_without_leakage(client, monkeypatch, observed_at, private_marker):
    response = assert_publication_fails_closed(
        client,
        monkeypatch,
...
def test_canonical_observed_at_variants_remain_publishable(client, monkeypatch, identity_class, relation, observed_at):
    calls = []
...
def test_contradictory_or_unsupported_decision_fails_closed(client, monkeypatch, invalid_decision):
    monkeypatch.setattr("app.blueprints.agent.resolve_runtime_current_entitlement", lambda _subject: invalid_decision)
    response = client.get(ENDPOINT)
    assert response.status_code == 503
    assert response.get_json() == {"error": "entitlement_unavailable"}
...
def test_standalone_request_does_not_wrap_resolver_in_session_scope(client, monkeypatch):
    ...
    monkeypatch.setattr("app.blueprints.agent.resolve_runtime_current_entitlement", lambda _subject: decision())
