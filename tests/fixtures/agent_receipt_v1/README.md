...
The `receipt_signed.json` signature is an illustrative placeholder. It is not a valid production signature unless a test explicitly proves it against the included `agent_pubkey`; the current fixture tests do not make that claim. Live signatures must be verified against the receipt `agent_pubkey` from live receipts.
