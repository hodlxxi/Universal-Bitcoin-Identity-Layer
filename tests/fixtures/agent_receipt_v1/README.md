...
The 409 fixture includes a `qr_pointer` object whose `target_path` reopens `/agent/verify/<job_id>` as a discovery-only receipt verification surface. That pointer is not a QR route, not a receipt, and not proof of receipt validity or payment.
