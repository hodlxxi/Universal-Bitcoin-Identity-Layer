...
from app.services.canonical_admission_edge import (
    GRAPH_ID, HUMAN_PROFILE, NETWORK, CanonicalAdmissionEdge,
    canonical_admission_edge_bytes, canonical_admission_edge_sha256,
    parse_canonical_admission_edge,
)
from app.services.canonical_genesis_record import (
    PARTICIPANT_ID, CanonicalGenesisRecord, canonical_genesis_record_bytes,
    canonical_genesis_record_sha256, parse_canonical_genesis_record,
)
...
from app.services.canonical_crt_authorization_proof import (
    CanonicalCrtAuthorizationProof, canonical_crt_authorization_proof_bytes,
    canonical_crt_authorization_proof_sha256, parse_canonical_crt_authorization_proof,
)
...
from app.services.covenant_relation import (
    CovenantRelationEvaluation, covenant_relation_source_sha256,
)
from app.services.trusted_covenant_registration import (
    TrustedCovenantRegistration, canonical_trusted_registration_bytes,
    trusted_outpoints_from_registration, trusted_registration_sha256,
)
from app.services.trusted_crt_authorization_source_plan import (
    TrustedCrtAuthorizationSourcePlan, TrustedCrtLineageSource,
...
from app.services.trusted_crt_bitcoin_observation_snapshot import (
    TrustedCrtBitcoinObservationResolution, TrustedCrtBitcoinObservationSnapshot,
    TrustedCrtBitcoinObservationState, TrustedCrtObservedLineageRelation,
...
STATUS = (
    "IMPLEMENTED_DORMANT", "HODLXXI_V1_PROFILE_SPECIFIC",
...
EXPLICIT_NON_CLAIMS = (
    "no repository lookup", "no database read or write", "no Bitcoin RPC call",
    "no LND call", "no network request", "no participant or target discovery",
    "no fabricated Bitcoin observation", "no descriptor import or custody proof",
    "no transaction broadcast", "no signing", "no issuer attestation",
    "no authority-legitimacy proof", "no proof of private-key possession",
    "no portable trust bundle", "no replica synchronization", "no graph checkpoint",
    "no cross-node fork detection", "no universal freshness claim",
    "no claim after freshness_deadline", "no JWT or session issuance",
    "no FULL/LIMITED session mutation", "no entitlement evidence write",
    "no action authorization", "no administrator or sponsor permission grant",
    "no HTTP route", "no MCP tool", "no paid job", "no CLI", "no scheduler",
    "no migration", "no deployment", "no replacement of the active legacy runtime path",
)
...
def _evaluation(value: object) -> CovenantRelationEvaluation:
    ...
    try:
        return CovenantRelationEvaluation(*(
            deepcopy(getattr(value, field))
            for field in CovenantRelationEvaluation.__dataclass_fields__
        ))
    except Exception:
        _fail()
...
def _plan(value: object) -> TrustedCrtAuthorizationSourcePlan:
    if type(value) is not TrustedCrtAuthorizationSourcePlan:
        _fail()
    try:
        genesis = tuple(parse_canonical_genesis_record(canonical_genesis_record_bytes(x))
                        for x in value.genesis_records)
        sources = []
        ...
        detached = TrustedCrtAuthorizationSourcePlan(
            value.schema, value.adapter_version, value.graph_or_protocol_id,
            value.subject_kind, value.participant_id, value.compressed_public_key,
            value.x_only_public_key, value.depth, value.target_edge_id,
            value.target_edge_sha256, genesis, tuple(sources),
            deepcopy(value.relevant_records), value.manifest_sha256,
            deepcopy(value.explicit_non_claims), value.human_interpretation_required,
        )
...
def _resolution(value: object, plan: TrustedCrtAuthorizationSourcePlan):
    ...
    for relation in snapshot.observed_relations:
        ...
        relations.append(TrustedCrtObservedLineageRelation(
            relation.depth, relation.edge_id, relation.edge_sha256,
            relation.registration_id, relation.registration_sha256,
            deepcopy(relation.trusted_outpoints), relation.trusted_outpoints_sha256,
            _evaluation(relation.relation_evaluation), relation.relation_evaluation_sha256,
        ))
    detached = TrustedCrtBitcoinObservationSnapshot(
        snapshot.schema, snapshot.adapter_version, manifest, plan,
        snapshot.graph_or_protocol_id, snapshot.participant_id,
        snapshot.target_edge_id, snapshot.source_plan_depth,
        snapshot.observed_block_height, snapshot.observed_best_block_hash,
        snapshot.observation_started_at, snapshot.observation_completed_at,
        tuple(relations), snapshot.snapshot_sha256,
        deepcopy(snapshot.explicit_non_claims), snapshot.human_interpretation_required,
    )
...
def canonical_crt_plan_identity(plan: TrustedCrtAuthorizationSourcePlan) -> tuple:
    return (
        plan.manifest_sha256, plan.graph_or_protocol_id, plan.participant_id,
        plan.target_edge_id, plan.depth,
        tuple((x.depth, x.edge_id, x.edge_sha256, x.registration_id,
               x.registration_sha256, x.observation_required) for x in plan.lineage_sources),
    )
...
def _evidence_payload(evidence, plan_digest, snapshot_digest):
    return {
        "graph_or_protocol_id": GRAPH_ID, "human_profile": HUMAN_PROFILE,
        ...
        "edge_evidence": [{
            ...
            "relation_evaluation_sha256": (None if item.observation_evaluation is None else
                covenant_relation_source_sha256(item.observation_evaluation)),
...
def hodlxxi_v1_edge_evidence_manifest_bytes(
    ...
    if len(edge_evidence) != len(plan.lineage_sources):
        _fail()
    return json.dumps(_evidence_payload(edge_evidence, plan,
        bitcoin_snapshot_sha256), sort_keys=True, separators=(",", ":"),
...
def _detach_evidence(value: object) -> tuple[CanonicalSponsorLineageEdgeEvidence, ...]:
    ...
    for item in value:
        ...
        evaluation = None if item.observation_evaluation is None else _evaluation(
            item.observation_evaluation)
...
def _authoritative_evidence(
    ...
    relations = {} if resolution.snapshot is None else {
        (item.depth, item.edge_id, item.edge_sha256, item.registration_id,
         item.registration_sha256): item
        for item in resolution.snapshot.observed_relations
    }
    if resolution.snapshot is not None and len(relations) != len(
            resolution.snapshot.observed_relations):
        _fail()
    evidence = []
    for source in plan.lineage_sources:
        key = (source.depth, source.edge_id, source.edge_sha256,
               source.registration_id, source.registration_sha256)
        relation = relations.pop(key, None)
        if source.observation_required is not (relation is not None):
            _fail()
        evidence.append(CanonicalSponsorLineageEdgeEvidence(
            source.edge, source.registration,
...
class HodlxxiV1SnapshotProofCompositionResult:
    ...
    def __post_init__(self):
        if type(self) is not HodlxxiV1SnapshotProofCompositionResult:
            _fail()
        if (self.schema != SCHEMA or self.composer_version != COMPOSER_VERSION
                or self.graph_or_protocol_id != GRAPH_ID or self.network != NETWORK
        ...
        if resolution.state is TrustedCrtBitcoinObservationState.NOT_REQUIRED:
            if any(value is not None for value in (
                    self.bitcoin_snapshot_sha256, self.observed_block_height,
                    self.observed_best_block_hash, self.observation_started_at,
                    self.observation_completed_at)):
                _fail()
        else:
            if snapshot is None or any(value is None for value in (
                    self.bitcoin_snapshot_sha256, self.observed_block_height,
                    self.observed_best_block_hash, self.observation_started_at,
                    self.observation_completed_at)):
                _fail()
        ...
        expected_snapshot_digest = (None if snapshot is None else
                                    trusted_crt_bitcoin_observation_snapshot_sha256(snapshot))
        expected_scalars = (
            trusted_crt_authorization_source_plan_manifest_sha256(plan), resolution.state,
            ...
            plan.participant_id, plan.target_edge_id, plan.depth,
        )
        if expected_scalars != (
            self.source_plan_manifest_sha256, self.bitcoin_observation_state,
            self.bitcoin_snapshot_sha256, self.observed_block_height,
            self.observed_best_block_hash, self.observation_started_at,
            self.observation_completed_at, self.participant_id,
            self.target_edge_id, self.depth,
        ) or detached_evidence != expected_evidence:
            _fail()
        if plan.subject_kind is TrustedCrtSubjectKind.GENESIS:
            if (resolution.state is not TrustedCrtBitcoinObservationState.NOT_REQUIRED
                    or plan.target_edge_id is not None or plan.depth != 0
        ...
        if (canonical_crt_authorization_proof_bytes(proof) != proof_bytes
...
def _result_payload(value):
    return {
        "schema": value.schema, "composer_version": value.composer_version,
        "graph_or_protocol_id": value.graph_or_protocol_id, "network": value.network,
        ...
        "participant_id": value.participant_id, "target_edge_id": value.target_edge_id,
        "depth": value.depth, "evaluated_at": _timestamp(value.evaluated_at),
...
def _result_bytes_unchecked(value):
    return json.dumps(_result_payload(value), sort_keys=True, separators=(",", ":"),
                      ensure_ascii=True, allow_nan=False).encode("ascii")
...
def hodlxxi_v1_snapshot_proof_composition_bytes(value) -> bytes:
    if type(value) is not HodlxxiV1SnapshotProofCompositionResult:
        _fail()
    validated = HodlxxiV1SnapshotProofCompositionResult(*(
...
def compose_hodlxxi_v1_snapshot_authorization_proof(
    ...
    if type(request) is not HodlxxiV1SnapshotProofCompositionRequest:
        _fail()
    request = HodlxxiV1SnapshotProofCompositionRequest(*(
        getattr(request, f) for f in request.__dataclass_fields__))
    ...
    evidence_digest = hodlxxi_v1_edge_evidence_manifest_sha256(evidence,
        source_plan=plan,
        bitcoin_snapshot_sha256=snapshot_digest)
    values = dict(
        schema=SCHEMA, composer_version=COMPOSER_VERSION,
        graph_or_protocol_id=GRAPH_ID, network=NETWORK, human_profile=HUMAN_PROFILE,
        genesis_participant_id=PARTICIPANT_ID,
        source_plan=plan, bitcoin_observation_resolution=resolution,
        ...
        depth=plan.depth, evaluated_at=request.evaluated_at,
        freshness_deadline=request.freshness_deadline, edge_evidence=evidence,
        edge_evidence_manifest_sha256=evidence_digest, authorization_proof=proof,
        authorization_proof_sha256=canonical_crt_authorization_proof_sha256(proof),
        authority_state=AUTHORITY_STATE, explicit_non_claims=EXPLICIT_NON_CLAIMS,
