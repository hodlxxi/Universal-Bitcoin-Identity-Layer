...
_CONCLUSION_BY_REASON = MappingProxyType({
    ...
    CanonicalCrtAuthorizationReason.EXACT_PARTICIPANT_MEMBERSHIP_FULL:
        CanonicalCrtAuthorizationProofConclusion.FULL_BY_EXACT_PARTICIPANT_MEMBERSHIP,
    CanonicalCrtAuthorizationReason.PROVISIONAL_MEMBERSHIP_LIMITED:
        CanonicalCrtAuthorizationProofConclusion.LIMITED_BY_PROVISIONAL_MEMBERSHIP,
    CanonicalCrtAuthorizationReason.EDGE_INACTIVE_MEMBERSHIP_LIMITED:
        CanonicalCrtAuthorizationProofConclusion.LIMITED_BY_TARGET_EDGE_INACTIVITY,
    CanonicalCrtAuthorizationReason.LINEAGE_INACTIVE_MEMBERSHIP_LIMITED:
        CanonicalCrtAuthorizationProofConclusion.LIMITED_BY_LINEAGE_INACTIVITY,
    CanonicalCrtAuthorizationReason.DISPUTED_MEMBERSHIP_LIMITED:
        CanonicalCrtAuthorizationProofConclusion.LIMITED_BY_DISPUTED_MEMBERSHIP,
...
_BASIS_BY_REASON = MappingProxyType({
    CanonicalCrtMembershipReason.EXACT_GENESIS_MEMBERSHIP_ACTIVE:
        CanonicalCrtAuthorizationProofBasis.GENESIS_RECORD,
    ...
    CanonicalCrtMembershipReason.GENESIS_PROVISIONAL:
        CanonicalCrtAuthorizationProofBasis.GENESIS_CONTROL,
    ...
    CanonicalCrtMembershipReason.MISSING_TARGET_EVIDENCE:
        CanonicalCrtAuthorizationProofBasis.TARGET_EVIDENCE,
    CanonicalCrtMembershipReason.MISSING_PARENT_EVIDENCE:
        CanonicalCrtAuthorizationProofBasis.LINEAGE_STRUCTURE,
...
def _canonical_source(value: object) -> CanonicalCrtAuthorizationEvaluation:
    ...
    try:
        return parse_canonical_crt_authorization_evaluation(
            canonical_crt_authorization_evaluation_bytes(value)
        )
    except Exception:
        raise InvalidCanonicalCrtAuthorizationProof(
...
def _explanation(
    authorization_class, authorization_reason, membership_state,
    membership_reason, proof_basis,
) -> str:
    return (
...
class CanonicalCrtAuthorizationProof:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            required_strings = (
                ...
                self.participant_id, self.compressed_public_key,
                self.x_only_public_key, self.source_evaluation_sha256,
            ...
            optional_strings = (
                self.target_edge_id, self.target_edge_sha256,
                self.controlling_edge_id, self.selected_genesis_record_id,
            ...
            if (
                ...
                or (self.controlling_depth is not None
                    and type(self.controlling_depth) is not int)
            ...
            if (
                ...
                or type(self.authorization_reason_code)
                is not CanonicalCrtAuthorizationReason
                ...
                or type(self.source_evaluation_kind)
                is not CanonicalCrtMembershipSourceKind
                or type(self.proof_conclusion)
                is not CanonicalCrtAuthorizationProofConclusion
            ...
            if (
                self.graph_or_protocol_id, self.network, self.human_profile,
                self.subject_kind, self.participant_id,
                self.compressed_public_key, self.x_only_public_key, self.depth,
                self.target_edge_id, self.target_edge_sha256, self.evaluated_at,
                self.authorization_class, self.authorization_reason_code,
                self.current_full_membership_satisfied, self.membership_state,
                self.membership_reason_code,
            ) != (
                source.graph_or_protocol_id, source.network, source.human_profile,
                source.subject_kind, source.participant_id,
                source.compressed_public_key, source.x_only_public_key,
                source.depth, source.target_edge_id, source.target_edge_sha256,
                source.evaluated_at, source.authorization_class,
                source.reason_code, source.current_full_membership_satisfied,
            ...
            if (
                self.source_evaluation_kind, self.source_evaluation_sha256,
                self.source_state, self.source_reason_code,
                self.controlling_depth, self.controlling_edge_id,
                self.selected_genesis_record_id,
                self.selected_genesis_record_sha256, self.relevant_records,
            ) != (
                membership.source_evaluation_kind,
                membership.source_evaluation_sha256, membership.source_state,
                membership.source_reason_code, membership.controlling_depth,
                ...
                raise InvalidCanonicalCrtAuthorizationProof(
                    "membership source binding"
                )
            authorization_digest = canonical_crt_authorization_evaluation_sha256(
                source
            )
            membership_digest = canonical_crt_membership_evaluation_sha256(
                membership
            )
            if (
                self.source_authorization_evaluation_sha256
                ...
                or self.source_membership_evaluation_sha256
                != source.source_membership_evaluation_sha256
            ):
            ...
            if (
                ...
                or self.canonical_explanation != _explanation(
                    source.authorization_class, source.reason_code,
                    membership.state, membership.reason_code, expected_basis,
                )
            ):
    ...
    def _validate_basis(self, membership) -> None:
        ...
        if basis is CanonicalCrtAuthorizationProofBasis.GENESIS_RECORD:
            valid = (
                ...
                and (
                    (
                        ...
                        is CanonicalCrtMembershipState.GENESIS_ACTIVE
                    ...
                    or (
                        ...
                        and self.membership_reason_code
                        not in _SOURCE_BINDING_REASONS
            ...
            valid = (
                ordinary and self.membership_state is CanonicalCrtMembershipState.ACTIVE
                and self.controlling_depth is None
                and self.controlling_edge_id is None
            ...
            valid = (
                ordinary and self.membership_reason_code
                is CanonicalCrtMembershipReason.MISSING_TARGET_EVIDENCE
            )
        elif basis is CanonicalCrtAuthorizationProofBasis.LINEAGE_STRUCTURE:
            valid = (
                ordinary and self.controlling_depth is None
                and self.controlling_edge_id is None
            )
        else:
            valid = (
...
def build_canonical_crt_authorization_proof(
    ...
    try:
        ...
        return CanonicalCrtAuthorizationProof(
            ...
            source.graph_or_protocol_id, source.network, source.human_profile,
            source.subject_kind, source.participant_id,
            source.compressed_public_key, source.x_only_public_key, source.depth,
            source.target_edge_id, source.target_edge_sha256, source.evaluated_at,
            source.authorization_class, source.reason_code,
            source.current_full_membership_satisfied,
            membership.state, membership.reason_code,
            membership.source_evaluation_kind,
            membership.source_evaluation_sha256, membership.source_state,
            membership.source_reason_code, membership.controlling_depth,
            membership.controlling_edge_id,
            membership.selected_genesis_record_id,
            membership.selected_genesis_record_sha256,
            membership.relevant_records, _CONCLUSION_BY_REASON[source.reason_code],
            basis, _explanation(
                source.authorization_class, source.reason_code,
                membership.state, membership.reason_code, basis,
            ),
            canonical_crt_authorization_evaluation_sha256(source),
            canonical_crt_membership_evaluation_sha256(membership),
            source, MANDATORY_NON_CLAIMS, True,
        )
    except InvalidCanonicalCrtAuthorizationProof:
        raise
    except Exception:
...
def _dict(value: CanonicalCrtAuthorizationProof) -> dict[str, object]:
    source = json.loads(
        canonical_crt_authorization_evaluation_bytes(
            value.source_authorization_evaluation
        ).decode("ascii")
    )
    payload = {field: getattr(value, field) for field in value.__dataclass_fields__}
    for field in (
        "subject_kind", "authorization_class", "authorization_reason_code",
        "membership_state", "membership_reason_code", "source_evaluation_kind",
        "proof_conclusion", "proof_basis",
    ):
        payload[field] = getattr(value, field).value
    payload["evaluated_at"] = (
        value.evaluated_at.isoformat(timespec="seconds").replace("+00:00", "Z")
    )
...
def canonical_crt_authorization_proof_bytes(
    ...
    try:
        ...
        return json.dumps(
            _dict(value), sort_keys=True, separators=(",", ":"),
    ...
    try:
        ...
        if (
            ...
            or any(type(item) is not list or len(item) != 2
                   for item in data["relevant_records"])
        ):
            raise ValueError()
        ...
        if type(timestamp) is not str or not timestamp.endswith("Z"):
            raise ValueError()
        nested = json.dumps(
            data["source_authorization_evaluation"], sort_keys=True,
        ...
        result = CanonicalCrtAuthorizationProof(**{
            **data,
            ...
            "authorization_class":
                CanonicalCrtAuthorizationClass(data["authorization_class"]),
            "authorization_reason_code":
                CanonicalCrtAuthorizationReason(data["authorization_reason_code"]),
            "membership_state": CanonicalCrtMembershipState(data["membership_state"]),
            "membership_reason_code":
                CanonicalCrtMembershipReason(data["membership_reason_code"]),
            "source_evaluation_kind":
                CanonicalCrtMembershipSourceKind(data["source_evaluation_kind"]),
            "relevant_records": tuple(
                tuple(item) for item in data["relevant_records"]
            ),
            "proof_conclusion":
                CanonicalCrtAuthorizationProofConclusion(data["proof_conclusion"]),
