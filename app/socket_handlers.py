...
def register_socket_handlers(socketio):
    ...
    def rtc_join_room(data):
        try:
            ...
            app_module.logger.info(
                f"User {app_module.truncate_key(pubkey)} joined room {room_id}. "
            ...
            if room_id.startswith("direct-") and len(room["pubkeys"]) == 1:
                ...
                if other_pubkey:
                    app_module.logger.info(
                        f"Auto-inviting {app_module.truncate_key(other_pubkey)} to join {room_id}"
                    )
    ...
    def rtc_invite(data):
        ...
        for sid in sids_for_pubkey(to_pubkey):
            emit(
                "rtc:invite",
                {"room_id": room_id, "from": from_pubkey, "from_name": from_pubkey[-8:]},
                room=sid,
            )
    ...
    def on_connect(auth=None):
        ...
        online_list = [
            {"pubkey": pk, "role": app_module.ONLINE_META.get(pk, "limited")}
            for pk in app_module.ONLINE_USERS
        ]
        try:
