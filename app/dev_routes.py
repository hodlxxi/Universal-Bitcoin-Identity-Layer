...
_PUBKEY_COMP_RE = re.compile(r"^(02|03)[0-9a-f]{64}$")
_PUBKEY_XONLY_RE = re.compile(r"^[0-9a-f]{64}$")


def normalize_pubkey_for_payments(db_session, pk: str) -> str:
    """
    payments.user_pubkey requires compressed pubkey hex (66 chars starting 02/03).
    If session has x-only (64 chars), try resolve by checking ubid_users for 02/03+X.
    """
    pk = (pk or "").strip().lower()
    if _PUBKEY_COMP_RE.match(pk):
        return pk
    if _PUBKEY_XONLY_RE.match(pk):
        for prefix in ("02", "03"):
            cand = prefix + pk
            row = db_session.execute(
                text("SELECT 1 FROM ubid_users WHERE pubkey=:p LIMIT 1"),
                {"p": cand},
            ).fetchone()
            if row:
                return cand
...
def require_billing_limited(f):
    """
    Step-6 billing model:
      - guests: cannot bill
      - full/special: free, cannot bill
      - only limited + real pubkey can PAYG top-up
    """
    ...
    def decorated_function(*args, **kwargs):
        ...
        if lvl == "guest" or str(user_pubkey).startswith(("guest_", "anon_", "guest-")):
            return jsonify({"ok": False, "error": "guests_cannot_use_billing"}), 403
        if lvl in {"full", "special"}:
            return jsonify({"ok": False, "error": "full_accounts_are_free"}), 403
        if lvl != "limited":
            return jsonify({"ok": False, "error": "billing_requires_limited_access"}), 403
...
@require_billing_limited
def create_invoice_route():
    """Create Lightning invoice for PAYG top-up (limited users only)."""
    user_pubkey = session.get("logged_in_pubkey")
    ...
    try:
        ...
        if plan in {"payg", "topup"}:
            ...
            return jsonify({"ok": False, "error": "only_payg_topups_supported"}), 400
        ...
        memo = "HODLXXI PAYG top-up"
        payment_request, invoice_id = create_invoice(amount_sats, memo, user_pubkey)

        with session_scope() as db_session:
            # Normalize pubkey to compressed form for payments constraint/FK

            user_pubkey = normalize_pubkey_for_payments(db_session, user_pubkey)

            session["logged_in_pubkey"] = user_pubkey
            # Enforce compressed pubkey format (payments constraint)
            if not re.match(r"^(02|03)[0-9a-f]{64}$", (user_pubkey or "").lower()):
                return jsonify({"ok": False, "error": "pubkey_must_be_compressed_66"}), 400
            # Ensure ubid_users row exists (payments FK)
            db_session.execute(
                text(
                    "INSERT INTO ubid_users (pubkey, plan, sats_balance, created_at, last_login_at, payg_enabled) "
                    "VALUES (:p, :plan, 0, NOW(), NOW(), false) "
                    "ON CONFLICT (pubkey) DO UPDATE "
                    "SET last_login_at = NOW()"
                ),
                {"p": user_pubkey, "plan": "limited"},
            )
...
def check_invoice_route():
    """Check if Lightning invoice has been paid; credit balance once."""
    user_pubkey = session.get("logged_in_pubkey")
    ...
    try:
        ...
        with session_scope() as db_session:
            ...
            payment = db_session.execute(
                text(
                    ...
                    "COALESCE((metadata->>'credited')::boolean,false) AS credited "
                    "FROM payments "
            ...
            plan, amount_sats, status, credited = payment[0], int(payment[1] or 0), payment[2], bool(payment[3])

            if status == "paid" and credited:
                return jsonify(
                    {
                        "paid": True,
            ...
            credited_row = db_session.execute(
                text(
                    ...
                    UPDATE payments
                       SET status='paid',
                           paid_at=:now,
                           metadata=(COALESCE(metadata,'{}'::jsonb) || jsonb_build_object('credited', true))::jsonb
        ...
        plan2, amount2 = credited_row[0], int(credited_row[1] or 0)
        return jsonify(
            {"paid": True, "invoice_id": invoice_id, "plan": plan2, "amount_sats": amount2, "credited_now": True}
        )
