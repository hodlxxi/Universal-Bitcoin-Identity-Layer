...
def require_login(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if str(user_pubkey).startswith("guest-"):
            return jsonify({
                "error": "Guests cannot access developer billing. Please login with a real key."
            }), 403
...
def get_or_create_dev_account(user_pubkey: str) -> Dict:
    ...
    with session_scope() as db_session:
        result = db_session.execute(
            text("SELECT * FROM dev_accounts WHERE user_pubkey = :pubkey"),
            {"pubkey": user_pubkey}
        ).fetchone()
        
        if result:
            return {
                "id": result[0], "user_pubkey": result[1], "plan": result[2],
                "status": result[3], "current_period_start": result[4],
                "current_period_end": result[5], "created_at": result[6],
                "updated_at": result[7]
            }
        
        db_session.execute(
            text("INSERT INTO dev_accounts (user_pubkey, plan, status, created_at) VALUES (:pubkey, 'free', 'active', :now)"),
    ...
    try:
        ...
        with session_scope() as db_session:
            db_session.execute(
                ...
                """),
                {
                    "pubkey": user_pubkey, "invoice_id": invoice_id,
                    "payment_request": payment_request, "amount": amount_sats,
                    "plan": plan, "now": datetime.now(timezone.utc),
        ...
        return jsonify({
            "ok": True, "payment_request": payment_request,
            "invoice_id": invoice_id, "amount_sats": amount_sats, "plan": plan
        })
    except Exception as e:
        ...
        if is_paid:
            with session_scope() as db_session:
                payment = db_session.execute(
                    text("SELECT plan, amount_sats, status FROM payments WHERE invoice_id = :invoice_id AND user_pubkey = :pubkey"),
                ...
                db_session.execute(
                    ...
                    """),
                    {"pubkey": user_pubkey, "plan": plan, "amount": payment[1],
                     "start": now, "end": period_end, "now": now}
                )
