...
def check_auth():
    ...
    # 1.5) Bearer token? Let @require_oauth_token handle it.
...
def on_disconnect(*args):  # Added *args
    ...
    # If no other sockets for this pubkey remain, drop presence
...
async function legacyVerify(){
  ...
  try{
    ...
    if(r.ok&&d.verified){sessionStorage.setItem('playLoginSound','1'); window.location.href='/app';}
...
async function guestLogin() {
  ...
  window.location.href = "/app"; // open chat directly
}
...
function startPolling(sid){
  ...
  poll=setInterval(async()=>{
    ...
    if(j.authenticated){clearInterval(poll);clearInterval(expire);
      closeQR();alert('Lightning login success!');window.location.href='/app';}
  },2000);
}
...
async function loginWithNostr(){
  ...
  if(j2.verified){alert('Nostr login success!');window.location.href='/app';}
...
def _public_guard_for_lnurl():
    ...
    # ⬇️ If a Bearer token is present, let the route's @require_oauth_token validate it
    auth = request.headers.get("Authorization", "")
    if auth.startswith("Bearer "):
        return None

    # Fallback: require session for other /api/*
    if p.startswith("/api/") and not session.get("logged_in_pubkey"):
...
<head>
  <meta charset="utf-8">
  <title>Playground - KeyAuth BTC OIDC</title>
  <style>
    :root{--bg:#0a0a0a;--fg:#e0e0e0;--accent:#00ff00}
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--fg);font-family:system-ui;padding:2rem}
    .container{max-width:1120px;margin:0 auto}
    a{color:var(--accent)}
    h1{color:var(--accent)}
    input,select{background:#141414;color:var(--fg);border:1px solid #333;padding:8px;border-radius:4px;width:100%;margin:4px 0}
    .btn{background:var(--accent);color:#000;padding:10px 20px;border:none;border-radius:8px;cursor:pointer;font-weight:bold}
...
<body>
  <div class="container">
    ...
    <form id="form">
      ...
      <input name="redirect_uri" placeholder="Redirect URI" value="http://localhost:3000/callback" required>
      <select name="scope">
        <option>read_limited</option>
        <option>read</option>
        <option>write</option>
      </select>
  ...
  <script>
    ...
    async function discover() {
      ...
      const data = await res.json();
      document.getElementById('discovery').textContent = JSON.stringify(data, null, 2);
    }
  </script>
</body>
</html>
