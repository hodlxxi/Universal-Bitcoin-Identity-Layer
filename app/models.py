...
class OAuthToken(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("id", "user_id", "client_id", name="uq_oauth_token_generation_owner"),
...
class Session(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("session_id", "user_id", name="uq_session_generation_owner"),
...
if TYPE_CHECKING:
    # The existing runtime Base is generated by declarative_base(). Describe
    # its ORM interface to static checkers without replacing its registry.
    from sqlalchemy.orm import DeclarativeBase as _OAuthSessionBase
else:
    _OAuthSessionBase = Base


class OAuthBrowserGeneration(_OAuthSessionBase):
    """Revocable browser login and retained, one-shot verified challenge commitment."""

    __tablename__ = "oauth_browser_generations"
    generation_id = Column(String(64), primary_key=True)
    user_id = Column(String(36), ForeignKey("users.id"), nullable=False)
    client_id = Column(String(255), ForeignKey("oauth_clients.client_id"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("generation_id", "user_id", "client_id", "subject", name="uq_browser_generation_owner"),
        CheckConstraint(_CanonicalLowerHex("generation_id", 64), name="ck_browser_generation_id"),
        CheckConstraint(_CanonicalLowerHex("proof_id", 64), name="ck_browser_generation_proof"),
        CheckConstraint(_CanonicalLowerHex("subject", 64), name="ck_browser_generation_subject"),
        CheckConstraint("created_at < expires_at", name="ck_browser_generation_lifetime"),
        Index("idx_browser_generation_owner", "user_id", "client_id"),
        Index(
            "uq_browser_generation_current",
            "user_id",
            "client_id",
            unique=True,
...
class OAuthSessionCodeBinding(_OAuthSessionBase):
    """Subject fixed at admitted authorization; no Session exists yet."""

    __tablename__ = "oauth_session_code_bindings"
    code = Column(String(255), ForeignKey("oauth_codes.code", ondelete="CASCADE"), primary_key=True)
    browser_generation_id = Column(String(64), ForeignKey("oauth_browser_generations.generation_id"), nullable=False)
    subject = Column(String(64), nullable=False)
    __table_args__ = (
        CheckConstraint(_CanonicalLowerHex("subject", 64), name="ck_oauth_session_code_subject"),
        Index("idx_oauth_code_browser", "browser_generation_id"),
    )


class OAuthSessionGeneration(_OAuthSessionBase):
    """Immutable relation, never a bearer or a replacement token store."""

    __tablename__ = "oauth_session_generations"
    token_id = Column(String(36), primary_key=True)
    browser_generation_id = Column(String(64), nullable=False)
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["browser_generation_id", "user_id", "client_id", "subject"],
            [
                "oauth_browser_generations.generation_id",
                "oauth_browser_generations.user_id",
                "oauth_browser_generations.client_id",
                "oauth_browser_generations.subject",
            ],
            name="fk_oauth_generation_browser_owner",
        ),
        ForeignKeyConstraint(
            ["token_id", "user_id", "client_id"],
            ["oauth_tokens.id", "oauth_tokens.user_id", "oauth_tokens.client_id"],
            name="fk_oauth_generation_token_owner",
        ),
        ForeignKeyConstraint(
            ["session_id", "user_id"],
            ["sessions.session_id", "sessions.user_id"],
            name="fk_oauth_generation_session_owner",
        ),
        CheckConstraint(_CanonicalLowerHex("subject", 64), name="ck_oauth_generation_subject"),
        Index("idx_oauth_generation_owner", "user_id", "client_id"),
        Index("idx_oauth_generation_browser", "browser_generation_id"),
    )
