...
def store_oauth_code(code: str, code_data: Dict, ttl=None, **kwargs) -> None:
    ...
    with session_scope() as session:
        ...
        if not user_id and user_pubkey:
            user = session.query(User).filter_by(pubkey=user_pubkey).first()
            if not user:
                user = User(pubkey=user_pubkey, metadata_json=code_data.get("user_metadata") or {})
                session.add(user)
                session.flush()
            user_id = user.id
        elif user_id:
            user = session.query(User).filter_by(id=user_id).first()
            if not user and user_pubkey:
                existing_by_pubkey = session.query(User).filter_by(pubkey=user_pubkey).first()
                if existing_by_pubkey:
                    user_id = existing_by_pubkey.id
                else:
                    user = User(id=user_id, pubkey=user_pubkey, metadata_json=code_data.get("user_metadata") or {})
                    session.add(user)
                    session.flush()
            elif not user and not user_pubkey:
                raise ValueError("OAuth code requires a valid user_id or user_pubkey")
