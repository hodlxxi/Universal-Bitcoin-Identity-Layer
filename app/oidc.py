...
def _agent_skill_docs(base: str) -> dict[str, str]:
    return {
        "mcp-server-card": (
            ...
            "- The live production endpoint is `https://hodlxxi.com/agent/mcp`.\n"
            "- nginx routes `/agent/mcp` to the dedicated loopback sidecar.\n"
            "- The Flask `/agent/mcp` route remains a fail-closed 501 fallback when nginx is not routing to the sidecar.\n"
            "- Local Streamable HTTP remains `http://127.0.0.1:8765/mcp`; stdio remains available for development.\n\n"
            "Safety:\n"
