...
def pof_stats_api():
    ...
    db = db_get_session()
    try:
        verified_users = (db.query(func.count(func.distinct(ProofOfFunds.user_id)))
                          .filter(ProofOfFunds.status=='verified')
                          .scalar()) or 0
        total_btc = (db.query(func.coalesce(func.sum(ProofOfFunds.total_btc), 0))
                     .filter(ProofOfFunds.status=='verified')
                     .scalar()) or 0
        addresses_verified = (db.query(func.coalesce(func.sum(ProofOfFunds.address_count), 0))
                              .filter(ProofOfFunds.status=='verified')
                              .scalar()) or 0
    finally:
        try: db.close()
