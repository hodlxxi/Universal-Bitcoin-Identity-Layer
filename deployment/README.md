...
> **Heads up:** The canonical deployment path now runs Gunicorn against `wsgi:app` under the `hodlxxi` user. If you swap process managers or change service names, update the generated unit files before running the script.
...
3. **App Service** – Install/enable your process manager unit.  The provided scripts should invoke `gunicorn --config /srv/app/scripts/gunicorn.conf.py wsgi:app` so production matches the factory-based test/runtime path.
