...
validate_scratch_name() {
  [[ "$SCRATCH_DATABASE" =~ ^ubid_restore_verify_[A-Za-z0-9_]+$ ]]
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --scratch-database)
      ...
      ;;
    --help)
      echo "usage: sudo bash scripts/postgres_verify_backup.sh --backup ABSOLUTE_PATH [--production-database NAME] [--postgres-os-user USER] [--scratch-database NAME]"
      exit 0
      ;;
    *)
...
object_counts() {
  runuser -u "$POSTGRES_OS_USER" -- psql -X -d "$1" -AtF '|' -c "SELECT count(*) FILTER (WHERE c.relkind IN ('r','p')), count(*) FILTER (WHERE c.relkind='S'), count(*) FILTER (WHERE c.relkind IN ('i','I')) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname NOT IN ('pg_catalog','information_schema') AND n.nspname NOT LIKE 'pg_toast%';"
}
